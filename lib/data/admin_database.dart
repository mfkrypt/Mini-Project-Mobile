...
class AdminDatabase {
  ...
  Future<List<String>> fetchDistinctBoothTypeNames() async {
    final db = await database;
    final rows = await db.rawQuery(
      'SELECT DISTINCT name FROM booth_types ORDER BY name',
    );
  ...
  Future<void> deleteBoothMap(int id) async {
    final db = await database;
    await db.transaction((txn) async {
      await txn.delete(
        'application_booths',
      ...
      await txn.delete('booth_maps', where: 'id = ?', whereArgs: [id]);
    });
  }

  Future<void> updateBoothMapType(int boothMapId, int boothTypeId) async {
    final db = await database;
    await db.update(
      'booth_maps',
  ...
  Future<List<String>> fetchApplicationAddonNames(int applicationId) async {
    final db = await database;
    final rows = await db.query(
      'application_addons',
      columns: ['name'],
  ...
  Future<List<ExhibitorApplication>> fetchOrganizerApplications(
    ...
    final rows = await db.rawQuery(
      '''
        ...
        LEFT JOIN booth_maps
          ON booth_maps.id = application_booths.booth_map_id
        LEFT JOIN users ON users.id = applications.exhibitor_id
