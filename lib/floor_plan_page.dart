...
class _FloorPlanPageState extends State<FloorPlanPage> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      booths = maps
          .map((booth) {
            final inCart = widget.cart.any((item) => item.id == booth.boothId);
            if (inCart) {
  ...
  void _addToCart(AdminBoothMap booth) {
    ...
    final already = widget.cart.any((item) => item.id == booth.boothId);
    if (already) {
      return;
    }
    setState(() {
          ...
          children: [
                    ...
                    children: [
                      ...
                      for (final booth in booths)
                        Positioned(
                          left: booth.x * 320,
                          top: booth.y * 200,
                          width: booth.width * 320,
                          height: booth.height * 200,
            ...
            if (imagePath == null)
              const Padding(
                ...
                child: Center(child: Text('No floor plan uploaded yet.')),
              ),
